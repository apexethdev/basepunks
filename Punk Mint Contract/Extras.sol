...
// File: Punk Mint Contract/DEGENPunks.sol
...
contract DEGENPunks is ERC721r, Ownable, ReentrancyGuard  {
...
enum PunkAttributeType {SEX, HAIR, EYES, BEARD, EARS, LIPS, MOUTH, FACE, EMOTION, NECK, NOSE, CHEEKS, TEETH}
    ...
    function constructTokenURI(uint16 tokenId) private view returns (string memory) {
        ...
        bytes memory title = abi.encodePacked("DPunk #", tokenId.toString());
    ...
    function tokenImage(uint16 tokenId) public view returns (string memory) {
        ...
        svgBytes.appendSafe('<svg width="1200" height="1200" shape-rendering="crispEdges" xmlns="http://www.w3.org/2000/svg" version="1.2" viewBox="0 0 24 24"><style>rect{width:1px;height:1px}</style><rect x="0" y="0" style="width:100%;height:100%" fill="#9271f0" /><g style="transform: translate(calc(50% - 12px), calc(50% - 12px))">');
